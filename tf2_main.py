...
def main():
    ...
    #inp = np.load('./cmu1107/x_train_ori_all_2048_1024.npy') # 2048, 1024
    #inp = np.load('./cmu1107/x_train_ori_all_1024_512.npy') # 1024, 512
    inp = np.load('./cmu1016/x_ori_all.npy') # source man, ori
    ...
    tar = np.load('./cmu1016/y_ori_all.npy') # target woman, ori
    
    
    #tar = np.load('./cmu1018/y_train_ori_man.npy')
    #tar = np.load('./cmu1018_2/y_train_ori_all.npy') 
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 5 ==0:
            ...
            fig1, ax = plt.figure(figsize=(10, 4))
            #plt.figure(figsize=(10, 4))
            librosa.display.specshow(librosa.amplitude_to_db(result_before, ref=np.max), y_axis='hz', x_axis='time', sr=16000, hop_length=args.hop)
            ...
            sf.write(wav_save_dir+name_before+'.wav', make_wav, 16000, format='WAV', endian='LITTLE', subtype='PCM_16')
            
            #train after (y_hat)
            fig2, ax = plt.figure(figsize=(10, 4))
            #plt.figure(figsize=(10, 4))
            librosa.display.specshow(librosa.amplitude_to_db(result_after, ref=np.max), y_axis='hz', x_axis='time', sr=16000, hop_length=args.hop)
            ...
            sf.write(wav_save_dir+name_after+'.wav', make_wav, 16000, format='WAV', endian='LITTLE', subtype='PCM_16')
            
            # real input (source)
            save_tar = tar[0]
            save_tar = np.transpose(save_tar, (1, 0))
            fig3, ax = plt.figure(figsize=(10, 4))
            #plt.figure(figsize=(10, 4))
            librosa.display.specshow(librosa.amplitude_to_db(save_tar, ref=np.max), y_axis='hz', x_axis='time', sr=16000, hop_length=args.hop)
            ...
            if not os.path.exists(wav_save_dir):
                os.makedirs(wav_save_dir)
            sf.write(wav_save_dir+real_name+'.wav', make_wav, 16000, format='WAV', endian='LITTLE', subtype='PCM_16')
