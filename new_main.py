...
parser.add_argument('--max_spec_length', type=int, default='438', help='number of spec max sequence size')
...
def create_masks(inp_txt, inp_spec, tar_txt, tar_spec):
    ...
    # enc_padding_mask_text = create_padding_mask_spec(inp_txt)
    enc_padding_mask_spec = create_padding_mask_spec(inp_spec)
    #print("enc_padding_mask_text is ", enc_padding_mask_text, np.shape(enc_padding_mask_text))
    #print("enc_padding_mask_spec is ", enc_padding_mask_spec, np.shape(enc_padding_mask_spec))
...
def input_fn(txt_inp, spec_inp, txt_dec, spec_dec, txt_tar, spec_tar, BATCH_SIZE, BUFFER_SIZE):
    # txt_dataset = tf.data.Dataset.from_tensor_slices((txt_inp, txt_inp)).map(tf_encode)
    # print(txt_dataset)
    dataset = tf.data.Dataset.from_tensor_slices((txt_inp, spec_inp, txt_dec, spec_dec, txt_tar, spec_tar))
...
def main():
    ...
    txt_enc_inp = np.load('./cmu1120/train_id_enc.npy')
    txt_dec_inp = np.load('./cmu1120/train_id_dec.npy')
    txt_tar_inp = np.load('./cmu1120/train_id_tar.npy')
    # inp_txt = txt_inp[:, 1:-1]
    # tar_txt = txt_inp
    ...
    train_dataset = input_fn(txt_enc_inp, enc_inp_spec, txt_dec_inp, dec_inp_spec, txt_tar_inp, tar_inp_spec, batch_size, buffer_size)
    ...
    def train_step(inp_txt, inp_spec, dec_txt, dec_spec, tar_txt, tar_spec):
        # tar_txt_inp = tar_txt[:, :-1] --> dec
        # tar_txt_real = tar_txt[:, 1:] --> tar
        # tar_spec_inp = tar_spec[:, :-1]  dec
        # tar_spec_real = tar_spec[:, 1:] --> tar
        ...
        enc_padding_mask, combined_mask, dec_padding_mask = create_masks(inp_txt, inp_spec, tar_txt, tar_spec)
        ...
        with tf.GradientTape() as text_tape, tf.GradientTape() as spec_tape:
            ...
            predict_txt, predict_spec, attention_weights = transformer(inp_txt, inp_spec, dec_txt, dec_spec,
                                                                       True,
                                                                       enc_padding_mask,
                                                                       combined_mask,
                                                                       dec_padding_mask)
    ...
    for epoch in range(EPOCHS):
        ...
        # inp -> man, tar -> woman
        for (batch, (inp_txt, inp_spec, dec_txt, dec_spec, tar_txt, tar_spec)) in enumerate(train_dataset):
